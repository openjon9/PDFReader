...
public class FileUtil {
    ...
    public static String PATH_PHOTOGRAPH = "/PDFReader/PDFReaderPic/";
...
//    }
    ...
    public static File getDCIMFile(String filePath, String imageName) {
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) { // 文件可用
            File dirs = new File(Environment.getExternalStorageDirectory(), filePath);
            if (!dirs.exists()) {
                dirs.mkdirs();
            }
            File file = new File(Environment.getExternalStorageDirectory(), filePath + imageName);
            if (!file.exists()) {
                try {
                    //在指定的文件夹中创建文件
                    file.createNewFile();
